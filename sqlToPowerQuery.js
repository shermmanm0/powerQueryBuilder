...
function getQueriesFromSQL(fileName){
    ...
    const tablesRegEx = new RegExp(/((from)\s[a-z_]+\s+[a-z_]+)|((inner join)\s[a-z_]+\s+[a-z_]+)/g);
    ...
    if(args!=null){
        args = args.map(x =>{return x.substring(1,x.length)});
    }
    ...
    let fieldsProcessed = fields.map(fieldString =>{
       ...
       fieldObject = {
            "table": tablesProcessed.find(table => table.abbreviation === fieldStringArray[0]).name,
