...
class Plugin{
	constructor(name){
		...
		this.queriesXMLFilePath = `${this.name}.queries.xml`
		this.versionNumber = 0;

	}
	//adds query
	addQuery(name,area,coreTable,description,flattened=false){
		let query = new PowerQuery(this.getNumberOfQueries(),name,area,coreTable,description,flattened);
	...
	addArgToQuery(index, name, column, defaultValue, description, type = "primitive", required = "true"){
		let temp = new PowerQueryArg(name, column, defaultValue, description, type, required)
	...
	getPluginXML(){
		...
		pluginXML = pluginXML.concat(`<plugin xmlns="http://plugin.powerschool.pearson.com"\n`)
		pluginXML = pluginXML.concat(`\t\txmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"\n`)
		pluginXML = pluginXML.concat(`\t\txsi:schemaLocation="http://plugin.powerschool.pearson.com plugin.xsd"\n`)
		pluginXML = pluginXML.concat(`\t\tname=""\n`)
		pluginXML = pluginXML.concat(`\t\tversion=1.${this.versionNumber}\n`)
		pluginXML = pluginXML.concat(`\t\tdescription="">\n`)
		pluginXML = pluginXML.concat(`\t<oauth></oauth>\n`)
		pluginXML = pluginXML.concat(`\t<access_request>\n`)
		this.items.forEach(item =>{
			pluginXML = pluginXML.concat(`\t\t<field table ="${item.table}" field="${item.field}" access = "${item.access}"\n`)
		})
		pluginXML = pluginXML.concat(`\t</access_request>\n`)
		pluginXML = pluginXML.concat(`\t<publisher name ="">\n`)
		pluginXML = pluginXML.concat(`\t\t<contact email ="">\n`)
		pluginXML = pluginXML.concat(`\t</publisher>\n`)
		pluginXML = pluginXML.concat(`</plugin>\n`)
		return pluginXML
	}
	writeJSONToFile(){
		fs.writeFile(this.jsonFilePath,JSON.stringify(this,null,4),function(err){
			if (err){
				console.log(err);
			} else{
	...
	loadDataFromJSON(pathToJSON){
		...
		newData.array.forEach(element =>{
			...
			element.queryItems.forEach(items => {
				this.addItemToQuery(currentQueryIndex,items.name,items.table,items.field, items.access)
			})
			element.queryArgs.forEach(args => {
				this.addArgToQuery(currentQueryIndex,args.name,args.column,args.defaultValue,args.description,args.type,args.required)
			})
			currentQueryIndex++;
		})
	...
	addQueryFromJSON(object){
		let query = new PowerQuery(object.id,object.name,object.area,object.coreTable,object.description,object.flattened)
		if(this.checkQueryNameForUniqueness(query.getQueryName()))
		{
			this.array.push(query);
			console.log("Query added with ID "+query.getId());
...
class PowerQueryArg{
	constructor(name, column, defaultValue, description, type = "primitive", required = "true"){
		this.name = name;
		this.column = column;
